...
import { proofStart, installRequests, ProofRequest, unitFrame } from '../docs/render/rom/effect/proof.js';
...
// the allocator object as the harness lays it out (efx_emu.py ALLOC_OBJ, vtable at +0x1000: +0x1c and +0x20
// alloc, +0x34 free), for lifted code that frees through it (a proof start's reset)
m.w32(0x600f0000, 0x600f1000); m.w32(0x600f101c, 0x7e000000); m.w32(0x600f1020, 0x7e000000); m.w32(0x600f1034, 0x7e000104);
...
if (par){
  ...
  if (!info.proof && !info.request) m.load(owner + 0x30, new Uint8Array(new Uint32Array([P]).buffer));
  par.hex = hex;
}
...
} else if (!info.request && startEffect(m, owner) !== 1) throw new Error('start failed');
// a whole request (efx/proofunit.py): the boot objects, the core, and the unit passes every frame
let requestState = null;
if (info.request){
  const q = info.request;
  requestState = installRequests(m, malloc);
  const req = new ProofRequest(m, requestState, { list, parent: par.object, area: 1,
    record: { index: q.index, key: q.key, path: q.path, payload: Uint8Array.from(q.payload.match(/../g), b => parseInt(b, 16)) } });
  if (req.core !== q.core) throw new Error('request allocation diverged: core 0x' + req.core.toString(16) + ', harness 0x' + q.core.toString(16));
}
for (let f = 0; f < info.frames; f++){
  if (par) m.load(parentArray, par.hex(par.frames[f]));
  if (requestState) unitFrame(m, requestState);
  else move(m, owner);
}
